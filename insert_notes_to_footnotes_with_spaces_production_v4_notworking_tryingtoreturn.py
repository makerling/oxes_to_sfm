...
#Path('output3.txt').touch()
with open('output3.txt', 'w') as clearfile:
	print('clearing output3.txt file')
#open source file
with open('output.sfm', 'r') as file:
	f = file.read()
	#strip newlines before \p then after script put them back
	fstrip = re.sub(r'\n\\p',' \\p',f)	
	for fileline in fstrip:
		#if fileline.startswith(r'\\v'):
		matches = re.match(r'\\\\v\*\*\\\\v \d+==[^#]', fileline)
		if matches:
			print ('it matches!')
			#line3 = u'\\v**\\v 25==hicâb**GEN.2.25==####ve Âdem hem ȯnuŋ karısı ikisi dahi çıplak idiler ve hicâb ėtmezler idi ∞ '
			linesplit = fileline.split('==####')
			line2 = linesplit[0]
			line1 = line2.replace(' ','##').replace('\\\\','\\')
			print ('line 14',line1)
			print()
			line = unicodedata.normalize('NFC',line1)
			versetext1 = linesplit[1]
			print ('line 21',versetext1)
			print()
			...
			with open('output3.txt','a') as f:
				f.write(fileline)
#print ('finished')
#with open('output3.txt','r') as final:
#	finalf = final.read()
#
#finalf = re.sub(r' \\p',r'\n\\p',finalf)
